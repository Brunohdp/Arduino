/* ******************** Interruptor Clap Manual do Mundo ********************
   ...
   - Além das ligações com o Arduino também devem ser conectadas ao módulo relé as cargas que serão controladas por ele.  
     A entrada acontece pelo borne central de cada relé e a saída pelos laterais, variando entre direita ou esquerda 
     conforme a necessidade de ligar(normalmente aberto) ou desligar(normalmente fechado).
...
void setup() {
  ...
  // Inicia todos os relés na posição na qual os contatos estão desligados. Nosso módulo 
  //relé tem a lógica invertida HIGH desliga as portas, verifique se o usado por você também apresenta a mesma lógica.
